...
The first mobile surface is a UI lab, not production vault writing. `apps/mobile/src/components/ui` owns RNR-derived shadcn-style primitives such as text, buttons, and badges, while `apps/mobile/src/ui` owns Tolaria-specific wrappers such as icon buttons, panels, toolbars, list rows, chips, and property rows. Workspace components live under `apps/mobile/src/components/workspace` and consume `MobileWorkspaceSnapshot` from `apps/mobile/src/workspace/mobileWorkspaceModel.ts`. The default repository remains fixture-backed for screenshot speed, while the Playwright QA harness can inject a real read-only local-vault snapshot through `tolaria:mobile-workspace-snapshot` when `MOBILE_QA_VAULT_PATH` is set. `apps/mobile/src/workspace/localVaultSnapshot.ts` keeps that scan as a pure `LocalVaultFile[] -> MobileWorkspaceSnapshot` transform: it parses Tolaria frontmatter conventions, derives type colors from Type documents, caps rendered note rows for fast UI work, and leaves the source vault untouched.

Tablet editing is currently in-process and reducer-owned. `apps/mobile/src/workspace/mobileWorkspaceEditing.ts` applies note creation, title/body edits, frontmatter scalar property changes, relationship add/remove, archive, and favorite changes against the current `MobileWorkspaceSnapshot`. The reducer preserves frontmatter while the markdown editor edits body content, derives snippets/blocks/links back from Markdown, and resolves wikilink relationship targets against the snapshot. This gives the mobile UI real editing behavior for fixture and injected-vault QA without writing to disk yet. Native storage must enter later through a repository boundary that commits reducer results to the filesystem before updating the UI, matching the desktop disk-first invariant.
